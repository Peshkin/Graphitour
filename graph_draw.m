...
% 
% Input:  X -   N x 1 vector of x coordinates
...
%  [H, WIDTH] = TEXTBOX(X, Y, STR)     draws a box around the text 
%  [..] = TEXTBOX(STR)
% 
...
if ~isempty(ii),
	D1 = [pts(:,1+ii)-pts(:,9+ii) ...
	      ...
	      pts(:,8+ii)-pts(:,2+ii) ...
	      pts(:,9+ii)-pts(:,1+ii)]/2;
	ii = ii'*ones(1,8) + ones(length(ii),1)*[1:4 6:9];
	ii = ii(:)';
	pts(:,ii) = st(:,ii) + D1;
end;
